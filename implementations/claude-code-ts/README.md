...
> `--engine claude-code` for new deployments. Agent Hub now spawns the ACP proxy
> by default.
...
key**. Open the "Add remote agent" screen in the Shepaw app, copy the
shown base64 public key, then run on the agent host:

```sh
shepaw-claude-code peers add <base64-pubkey-from-app> --label "My iPhone"
```

The running gateway picks up the new entry within ~100 ms — no
restart. Then paste the banner's `ACP WS:` URL (**including the `#fp=`
fragment**) into Shepaw and connect. Try these messages:
...
```sh
shepaw-claude-code enroll --label "My iPhone" \
...
```sh
shepaw-claude-code enroll-list
shepaw-claude-code enroll-revoke 4B7-9KX-M2P
```
...
shepaw-claude-code peers list

# Authorize a new device (pubkey from the Shepaw app)
shepaw-claude-code peers add <base64-pubkey> --label "Alice's iPhone"

# Revoke a device by its 16-hex fingerprint
shepaw-claude-code peers remove a1b2c3d4e5f6a7b8
```
...
```sh
# Minimal, LAN only
shepaw-claude-code serve --cwd ~/code/my-project --port 8090
...
- **Running multiple gateways on one host.** The default identity path
  ...
  `shepaw-codebuddy-code-gateway`. If you run both on the same machine,
  give each its own identity file:

  ```sh
  SHEPAW_IDENTITY_PATH=~/.config/shepaw-claude-gateway/identity.json \
  SHEPAW_PEERS_PATH=~/.config/shepaw-claude-gateway/authorized_peers.json \
    shepaw-claude-code serve …
  ```
...
When Claude tries to use a tool you'll see a confirmation bubble on
your phone (Allow / Deny). Tapping **Allow** (or typing `allow` /
`同意`) on your next message lets Claude proceed; the approval is
