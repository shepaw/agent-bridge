 ...
 *
 * Non-blocking flow:
 *   1. Claude tries to use a tool → `canUseTool` callback fires.
 ...
 *   4. `ClaudeCodeAgent.onChat` detects the approval keyword and writes
 *      `{tool: 'Bash', inputHash: ...} → 'allow'` into this cache.
 *   5. The chat also forwards the user message to Claude, which retries
