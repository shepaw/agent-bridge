/**
 * Scripted replacement for `@anthropic-ai/claude-agent-sdk`'s `query()`
 * so the gateway can run end-to-end without an ANTHROPIC_API_KEY.
 *
  ...
  if (kw.includes('help')) {
    ...
    if (canUseTool !== undefined) {
      const decision = await canUseTool(
        'Bash',
        { command: 'echo "hello from mock Claude"' },
        { signal: signal ?? new AbortController().signal, toolUseID: `tu_${randomUUID().slice(0, 8)}` },
      );
      if (decision?.behavior === 'allow') {
        yield assistantToolUse('Bash', { command: 'echo "hello from mock Claude"' });
...
const HELP_TEXT = `[mock] Claude Code mock agent — try these in your next message:
...
function systemInit(sessionId: string): Record<string, unknown> {
  return {
    ...
    mcp_servers: [],
    model: 'claude-mock',
    claude_code_version: '0.0.0-mock',
  };
}
