 ...
 *
 * Storage: `~/.config/shepaw-cb-gateway/pending-approvals.json`, keyed by
...
const DEFAULT_PATH = join(homedir(), '.config', 'shepaw-cb-gateway', 'pending-approvals.json');

export interface PendingMarkerOptions {
  /** Override the persistence path. Default `~/.config/shepaw-cb-gateway/pending-approvals.json`. */
  path?: string;
}
