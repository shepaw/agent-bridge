 ...
 *
 * The directory layout is owned by the concrete agent (CodexAgent points at
 * `.codex/commands/` plus `~/.codex/commands/`). This module is agent-agnostic
 * — it just reads a directory and returns `SlashCommandInfo` entries tagged
 * with whatever `scope` the caller chose.
 *
...
import type { CommandScope, SlashCommandInfo } from 'shepaw-acp-sdk';
 ...
 * keys are ignored. Only string values are supported.
 */
  ...
  for (const raw of block.split(/\r?\n/)) {
    ...
    switch (key) {
      ...
      case 'argumentHint':
        ...
        break;
      default:
        // other known-but-unused keys silently ignored
        break;
    }
  }
  return out;
 ...
 * - Missing / unreadable directories return `[]`.
